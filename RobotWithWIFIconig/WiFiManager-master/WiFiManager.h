...
const char HTTP_FORM_START[] PROGMEM      = "<form method=\"get\" action=\"wifisave\"><label>SSID</label><input id=\"s\" name=\"s\" length=32 placeholder=\"SSID\"><label>Password (TEMP text)</label><input id=\"p\" name=\"p\" length=64 placeholder=\"password\">";
...
const char HTTP_FORM_IP_LABEL[] PROGMEM      = "<label for=\"{ip}\">{p}</label>";
const char HTTP_FORM_IP_PARAM[] PROGMEM      = "<input id=\"{i}\" name=\"{n}\" length={l} placeholder=\"{p}\" value=\"{v}\" {c}>";
...
class WiFiManager
{
  ...
  private:
	...
	const char *myHostname = "robot.com";
